...
This branch is stacked on safety PR #75. Merge #75 only after separate production approval, then rebase/retarget this PR to main and re-run comparisons. Do not skip the backup, immutable-image, lock and health-check changes.
...
- [ ] Merge safety PR #75 under its separate approval, then retarget this stacked PR.
