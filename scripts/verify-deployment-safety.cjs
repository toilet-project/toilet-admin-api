...
const step = steps.find(s => s.name === 'Deploy to Mini PC through Tunnel');
assert.ok(step, 'Reviewed Tunnel transport must exist');
assert.equal(step.env.TUNNEL_SSH_HOST, '${{ vars.TUNNEL_DEPLOY_SSH_HOST }}');
const source = step.env.DEPLOY_SCRIPT;
