...
test('moves static navigation and assets below the isolated admin preview path', () => {
  ...
  assert.match(html, /src="\/admin\/admin-preview-runtime\.js\?v=1"/)
  assert.match(html, /href="\/admin\/reports\.html"/)
  assert.match(html, /src="\/admin\/dashboard\.js"/)
  assert.match(html, /returnTo=preview/)
})
...
test('moves JavaScript page navigation without rewriting API paths', () => {
  ...
  assert.match(output, /assign\('\/admin\/'\)/)
  assert.match(output, /'\/admin\/reports\.html\?id=1'/)
  assert.match(output, /'\/api\/admin\/v1\/dashboard'/)
})

test('deployment route is limited to the admin path on the existing preview host', async () => {
  ...
  assert.deepEqual(config.routes, [{ pattern: 'preview.geupddong.com/admin*', zone_name: 'geupddong.com' }])
