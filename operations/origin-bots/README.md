...
Cloudflare 차단 통계가 아니라 **미니 PC의 기존 Nginx access.log에 기록된 요청**만 별도로 집계한다. 서버에 오지 않은 CDN 캐시 응답·차단 요청은 포함하지 않는다. 운영 설치·타이머 활성화·컨테이너 연결은 아직 하지 않았다.
...
5. 관리자 `ORIGIN_BOTS_DIRECTORY=/var/lib/geupddong-origin-bots`와 위 읽기 전용 디렉터리 바인드를 함께 준비한다. 배포 워크플로 연결은 별도 단계다. 기본 빈 설정이면 기존 페이지는 계속 동작한다.
...
봇 제외 API 보완은 별도 API 변경안이다. 기존 bot/crawler 필터에 Yeti·네이버 전용 로봇 등을 추가하며 **분석 이벤트 저장만 제외**한다. Nginx 요청 건수를 방문자 수에서 빼거나 과거 기록을 삭제하지 않는다. 요청에 202가 반환돼도 분석 테이블에 저장됐다는 뜻은 아니다.
